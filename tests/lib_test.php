...
final class lib_test extends \advanced_testcase {
     ...
     *
     * @covers ::aireading_add_instance
     * @covers ::aireading_delete_instance
     * @return void
     */
    public function test_create_delete_module(): void {
        ...
        $mod = $this->getDataGenerator()->create_module('aireading', ['course' => $course->id]);
        $cm = get_coursemodule_from_instance('aireading', $mod->id);

        // Assert it was created.
        $this->assertNotEmpty(\context_module::instance($mod->cmid));
        $this->assertEquals($mod->id, $cm->instance);
        $this->assertEquals('aireading', $cm->modname);
        $this->assertEquals(1, $DB->count_records('aireading', ['id' => $mod->id]));
        $this->assertEquals(1, $DB->count_records('course_modules', ['id' => $cm->id]));

        // Delete module.
        course_delete_module($cm->id);
        $this->assertEquals(0, $DB->count_records('aireading', ['id' => $mod->id]));
     ...
     *
     * @covers \backup_aireading_activity_structure_step
     * @covers \restore_aireading_activity_structure_step
     * @return void
     */
    public function test_backup_restore(): void {
        ...
        $this->assertEquals('aireading', $cmnew->modname);

        $name = $DB->get_field('aireading', 'name', ['id' => $cmnew->instance]);
